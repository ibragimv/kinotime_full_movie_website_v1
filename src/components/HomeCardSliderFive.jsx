...
const HomeCardSliderFive = () => {
  ...
  useEffect(() => {
    const fetchRandomMovies = async () => {
      try {
        const res = await fetch('https://api.kinotime.world/api/genres/10');
    ...
    <div className="slider-container-1">
      <div className="slider-header">
        <h1 className="slider-title">
          <BiGridAlt size={28} /> MULTFILMLAR
        </h1>
        <button className="see-all-button" onClick={() => navigate('/catalog/movie?genreId=10')}>
          BARCHASINI KO'RISH ➝
        </button>
      </div>
...
export default HomeCardSliderFive;
